...
def blrObjFunction(initialWeights, *args):
    ...
    Prob1 = sigmoid(np.dot(train_data, W))
    Prob2 = 1.0 - Prob1

    Prob1Like = np.multiply(np.log(Prob1),labeli)
    Prob2Like = np.multiply(np.log(Prob2),(1.0-labeli))

    Prob1 -= labeli
    Prob1 = np.expand_dims(Prob1, 1)
    error = np.sum(Prob1Like + Prob2Like) / -n_data
    error_grad = np.multiply(np.dot(Prob1.T, train_data) , n_data**-1)
...
def blrPredict(W, data):
    ...
    Prob1 = sigmoid(np.dot(data, W))
    Prob1Like = np.log(Prob1)
    Prob1Like -= np.ones_like(Prob1Like)

    label = np.argmax(Prob1Like, 1)
...
def mlrObjFunction(params, *args):
    ...
    multiply = np.dot(train_data, W)
    Prob1 = np.exp(multiply)
    Prob1Sum = np.expand_dims(np.sum(Prob1, 1),1)
    Prob1 = Prob1 * Prob1Sum**-1
    Prob1Like = Prob1
    error = - (np.sum(labeli * Prob1Like) * n_data**-1)
    error_grad = np.dot(train_data.T, (Prob1Like - labeli)) * n_data**-1
    error_grad = error_grad.flatten()
...
def mlrPredict(W, data):
    ...
    train_data = np.hstack((np.ones((n_data, 1)), train_data))
    Prob1 = np.zeros((train_data.shape[0], 10))


    Prob1 = np.exp(np.dot(train_data,initialWeights))
    Prob1Sum = np.sum(Prob1, 1)
    Prob1Sum = np.expand_dims(np.sum(Prob1, 1),1)
    Prob1 = Prob1/Prob1Sum

    label = np.argmax(Prob1, 1)
    label = np.expand_dims(label,1)
