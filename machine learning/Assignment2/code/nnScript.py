...
t0 = time.time() #time taken for execution
...
def initializeWeights(n_in, n_out):
    ...
    epsilon = sqrt(6) / sqrt(n_in + n_out + 1) #For a uniform distribution U(−a,a) we should take a=√6/ni+no+1 since Var(U(−a,a))=a2/3.
    W = (np.random.rand(n_out, n_in + 1) * 2 * epsilon) - epsilon
    return W


def sigmoid(z):
    """# Notice that z can be a s calar, a vector or a matrix
...
def preprocess():
    ...
    train_preprocess = np.zeros(shape=(50000, 784)) #50000 images are present in the file, the second is the size of image (28X28 = 784)
    ...
    # Feature selection
    feature_reduction=np.all(train_data == train_data[0,:] , axis=0)
    train_data=train_data[:,np.invert(feature_reduction)];
    validation_data=validation_data[:,np.invert(feature_reduction)];
    test_data=test_data[:,np.invert(feature_reduction)];
    print('No of features after reduction: ' + str(train_data.shape[1]))
    # Your code here.
...
#backpropagation
    ...
    # Calculate gradience
    gradOl = np.dot(np.transpose(deltaOl), zj)
    gradHl2 = np.dot(deltaOl,w2) * ((1 - zj) * zj)
    gradHl = np.dot(np.transpose(gradHl2), training_data)
    gradHl = gradHl[0:n_hidden, :]
...
#regularisation
   ...
   # Calculate Regularization values
    reg = J + (lambdaval/(2*len(training_data)))*(np.sum(w1*w1) + np.sum(w2*w2))
    obj_val = reg
    ...
    # obj_grad = np.concatenate((grad_w1.flatten(), grad_w2.flatten()),0)
    #obj_grad = np.array([])
    regHl = ((gradHl) + (lambdaval * w1)) / len(training_data)
    regOl = ((gradOl) + (lambdaval * w2)) / len(training_data)
   
    obj_grad = np.concatenate((regHl.flatten(), regOl.flatten()), 0)
