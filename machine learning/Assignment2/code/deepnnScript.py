...
def create_multilayer_perceptron():
    # Network Parameters
    n_hidden_1 = 256  # 1st layer number of features (nodes)
 ...
 #   n_hidden_6 = 256
 #   n_hidden_7 = 256
    n_input = 2376  # data input(batch size)
    n_classes = 2
    ...
    #added after weights --- (inputdata * weights)+biases
    biases = {
    ...
    }  #useful when all inputs are 0
    # tf Graph input
    x = tf.placeholder("float", [None, n_input]) #height,width
    y = tf.placeholder("float", [None, n_classes])


    # Hidden layer with RELU activation
    layer_1 = tf.add(tf.matmul(x, weights['h1']), biases['b1'])
    layer_1 = tf.nn.relu(layer_1) #activation func -rectified linear
    # Hidden layer with RELU activation
